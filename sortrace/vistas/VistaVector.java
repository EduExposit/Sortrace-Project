...
public class VistaVector extends JComponent {
    private Algoritmo algoritmo = null;
    private Color color = null;
    private Color colorParpadeo = null;
    private Color colorMinimo = null;
    private Color colorComparado = null;
    ...
    public void actualizarVector() {
        ...
        this.colorParpadeo = Color.YELLOW;
        this.colorMinimo = Color.GREEN;
        this.colorComparado = Color.CYAN;
        this.repaint();
    }
        ...
        if(algoritmo.getVector()==null){
            ...
            for (int i = 0; i < algoritmo.getVector().length; ++i) {
                if (algoritmo.esPosicionIntercambiada(i)) {
                    ...
                    g2D.setColor(this.colorParpadeo);
                    ...
                    if (algoritmo.noIntercambio()) {
                        g2D.setColor(this.colorComparado);
                        g2D.fillRoundRect(50, posYElemento - 10, anchoRect, 13, 5, 5);
                        g2D.setStroke(pincelNormal);
                        g2D.setColor(colorDefecto);
                    } else {
            ...
            g2DStadistics.drawString("Rendimiento:", 55 + anchoRect, 20);
            g2DStadistics.drawString("columnas" + " = " + algoritmo.Columnas(), 60 + anchoRect, 40);
            g2DStadistics.drawString("comparaciones" + " = " + algoritmo.Comparaciones(), 60 + anchoRect, 60);
            g2DStadistics.drawString("asignaciones" + " = " + algoritmo.Asignaciones(), 60 + anchoRect, 80);

            if(this.algoritmo instanceof Shell){
                Shell alg = (Shell) algoritmo;
                g2DStadistics.drawString("incrementos" + " = " + alg.getIncrementos(), 60 + anchoRect, 120);
                g2DStadistics.drawString("iteracion" + " = " + alg.getItActual()+"/"+alg.getItTotales(), 60 + anchoRect, 140);
            }
        }
    }
}
